...
export default class DetailsContactScreen extends React.Component {
  ...
  async getContactsList() {
    ...
    const response = await fetch(
      `${this.state.requestUrl}/${this.state.user_id}`,
      request,
    );
  ...
  async respondToContactRequest(targetId, didAccept) {
    const baseUrl =
      'http://ec2-34-214-245-195.us-west-2.compute.amazonaws.com:6464/notif/respondToContactRequest';
    const body = {
            ...
            {this.state.contactsArr.map((contact, index) => {
              ...
              return (
                <View style={cellStyle} key={index}>
                  <Text testID={'NameInfos_detox'} style={styles.titleText}>
                    {' '}
                    {`${contact.first_name} ${contact.last_name}`}
                  </Text>
                  <View style={styles.detailsView}>
                    <Text>{`Status: ${status}`}</Text>
                    <Text testID={'EmailInfos_detox'}>
                      {showContactInfo == 0
                        ? ``
                        : `Email: ${
                            contact.email == null
          ...
          <ScrollView
            ...
            {this.state.contactsArr.map((contact, index) => {
              ...
              if (showContactInfo == 0) {
                ...
                return (
                  <View style={cellStyle} key={index}>
                    <Text testID={'NameInfo_detox'} style={styles.titleText}>
                      {' '}
                      {`${contact.first_name} ${contact.last_name}`}
                    </Text>
                    <View style={styles.detailsView}>
                      <Text>{`Status: ${status}`}</Text>
                      <Text testID={'EmailInfo_detox'}>
                        {showContactInfo == 0
                          ? ``
                          : `Email: ${
                              contact.email == null

...
app.get('/testFirebase', async (req, res) => {
	try {
		...
		const targetDeviceToken =
			'eacSV4PETuaKMxuhSddbQE:APA91bHwt4gAjzwCgc6hqWnYJTjZWHdgKO3VWNfSGDwjnE9rMf3mWkhNS1VtrbA1pPNHJEL52n6uszKaPoE0forThY1u5YXzZUb3dvOvWBR3K9oepYQWk4qBgdXWbbtJR-SyMCnBwOzW';
		...
		const response = await fadmin
			.messaging()
			.sendToDevice(targetDeviceToken, payload);
		res.send('Done');
	} catch (err) {
		console.error(err);

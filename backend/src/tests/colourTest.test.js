...
test('light on white positive', async done => {
	colour = await getColour(
		'lostpetpictures',
		'pug_light_coat_white_background.jpg'
	);
...
test('black on black negative', async done => {
	colour = await getColour(
		'lostpetpictures',
		'Labrador_Retriever_thinks_is_pig_black_on_black_background.jpg'
	);
	expectedColour = [240, 210, 200];
	result = withinTolerance(expectedColour, colour.finalColor);
	expect(result).toBe(false);
...
test('grey on green positive', async done => {
	colour = await getColour(
		'lostpetpictures',
		'Egyptian_Cat_dark_green_background.jpg'
	);
...
test('brown on green positive', async done => {
	colour = await getColour(
		'lostpetpictures',
		'beagle_brown_on_green_background.jpg'
	);

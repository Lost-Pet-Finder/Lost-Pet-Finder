const allLostPets = [
	{
		information: {
			report_id: 6,
			fk_user_id: 4,
			file_name: 'Hare_brown_on_green_and_brown_background.jpg',
			location_x: 100,
			location_y: 100,
			report_date: '2020-03-03T10:00:00.000Z',
			tags:
				'["Mammal","Rodent","Animal","Hare","Kangaroo","Rat","Plant","Tree","Rabbit","Vegetation"]',
		},
		colours: {
			totalColor: [97, 93, 70],
			croppedColor: [107, 101, 78],
			finalColor: [111.25648813072765, 105.0809628469863, 81.07169246547356],
		},
	},
	{
		information: {
			report_id: 7,
			fk_user_id: 8,
			file_name: 'Labrador_Retriever_puppy_black_on_white_background.jpg',
			location_x: 100,
			location_y: -100,
			report_date: '2020-10-24T04:45:00.000Z',
			tags:
				'["Dog","Animal","Pet","Canine","Mammal","Labrador Retriever","Plant","Puppy"]',
		},
		colours: {
			totalColor: [236, 237, 238],
			croppedColor: [36, 35, 36],
			finalColor: [0, 0, 0],
		},
	},
	{
		information: {
			report_id: 8,
			fk_user_id: 9,
			file_name: 'golden_retriever_1.jpg',
			location_x: 100,
			location_y: -100,
			report_date: '2020-10-24T04:45:00.000Z',
			tags:
				'["Golden Retriever","Dog","Mammal","Pet","Animal","Canine","Plant","Grass","Puppy","Petal"]',
		},
		colours: {
			totalColor: [156, 158, 96],
			croppedColor: [158, 152, 102],
			finalColor: [159.23315999495415, 153.2489697384792, 102.75886768920255],
		},
	},
	{
		information: {
			report_id: 9,
			fk_user_id: 2,
			file_name:
				'Golden_Retriever_or_Labrador Retriever_gold_on_black_background.jpg',
			location_x: 100,
			location_y: -100,
			report_date: '2020-10-24T04:45:00.000Z',
			tags:
				'["Canine","Pet","Animal","Mammal","Dog","Puppy","Golden Retriever","Labrador Retriever"]',
		},
		colours: {
			totalColor: [6, 5, 5],
			croppedColor: [197, 181, 153],
			finalColor: [199.0583161637345, 182.71526346977876, 154.71526346977876],
		},
	},
];

const allFoundPets = [
	{
		information: {
			report_id: 25,
			fk_user_id: 3,
			file_name: 'Egyptian_Cat_dark_green_background.jpg',
			location_x: 123,
			location_y: -34,
			report_date: '2020-10-25T17:30:00.000Z',
			tags:
				'["Pet","Animal","Mammal","Cat","Jaguar","Panther","Wildlife","Leopard","Egyptian Cat"]',
		},
		colours: {
			totalColor: [39, 45, 18],
			croppedColor: [56, 60, 40],
			finalColor: [60.16373645804096, 64.80431129773957, 41.921724519095825],
		},
	},
	{
		information: {
			report_id: 30,
			fk_user_id: 5,
			file_name: 'pug_light_coat_white_background.jpg',
			location_x: 13,
			location_y: -55,
			report_date: '2020-10-23T16:30:00.000Z',
			tags: '["Canine","Pet","Mammal","Animal","Dog","Pug"]',
		},
		colours: {
			totalColor: [227, 211, 199],
			croppedColor: [228, 212, 200],
			finalColor: [228.08083745360108, 212.07513965951466, 200.07086631394984],
		},
	},
	{
		information: {
			report_id: 31,
			fk_user_id: 6,
			file_name: 'Labrador_Retriever_black_on_green_background.jpg',
			location_x: 55,
			location_y: -67,
			report_date: '2020-10-24T04:30:00.000Z',
			tags:
				'["Mammal","Pet","Labrador Retriever","Canine","Dog","Animal","Plant","Grass"]',
		},
		colours: {
			totalColor: [42, 54, 31],
			croppedColor: [40, 54, 31],
			finalColor: [39.53910389950093, 53.40741929935834, 30.659814782964972],
		},
	},
	{
		information: {
			report_id: 32,
			fk_user_id: 7,
			file_name: 'Hare_brown_on_green_and_brown_background.jpg',
			location_x: 100,
			location_y: -100,
			report_date: '2020-10-24T04:45:00.000Z',
			tags:
				'["Rodent","Hare","Animal","Mammal","Kangaroo","Rat","Plant","Tree","Rabbit","Vegetation"]',
		},
		colours: {
			totalColor: [97, 93, 70],
			croppedColor: [107, 101, 78],
			finalColor: [111.25648813072765, 105.0809628469863, 81.07169246547356],
		},
	},
	{
		information: {
			report_id: 36,
			fk_user_id: 1,
			file_name: 'image-5754bcc1-5919-4dee-85f6-8d74f6907345.jpg',
			location_x: 123,
			location_y: 123,
			report_date: '2020-10-10T16:13:00.000Z',
			tags: '["Green","First Aid","Plant","Art","Graphics"]',
		},
		colours: {
			totalColor: [240, 238, 180],
			croppedColor: [107, 130, 60],
			finalColor: [0, 0, 0],
		},
	},
	{
		information: {
			report_id: 104,
			fk_user_id: 4,
			file_name: 'beagle_brown_on_green_background.jpg',
			location_x: 100,
			location_y: 100,
			report_date: '2020-03-03T10:00:00.000Z',
			tags:
				'["Dog","Pet","Mammal","Canine","Animal","Hound","Strap","Beagle","Plant","Grass"]',
		},
		colours: {
			totalColor: [106, 107, 68],
			croppedColor: [131, 122, 84],
			finalColor: [139.9319531379669, 131.01621684681567, 89.72993220171462],
		},
	},
];

const intergrationLostPost = [
	{
		colour: '[97,93,70]',
		file_name: 'Hare_brown_on_green_and_brown_background.jpg',
		fk_user_id: 4,
		location_x: 100,
		location_y: 100,
		report_date: '2020-03-03T11:00:00.000Z',
		report_id: 6,
		tags:
			'[{"Name":"Mammal","Confidence":96.85425567626953,"Instances":[],"Parents":[{"Name":"Animal"}]},{"Name":"Hare","Confidence":96.85425567626953,"Instances":[],"Parents":[{"Name":"Rodent"},{"Name":"Mammal"},{"Name":"Animal"}]},{"Name":"Rodent","Confidence":96.85425567626953,"Instances":[],"Parents":[{"Name":"Mammal"},{"Name":"Animal"}]}]',
	},
];

const intergrationFoundPost = [
	{
		colour: '[139.9319531379669,128.15998464862733,89.72076431019188]',
		file_name: 'beagle_brown_on_green_background.jpg',
		fk_user_id: 4,
		location_x: 100,
		location_y: 100,
		report_date: '2020-03-03T11:00:00.000Z',
		report_id: 137,
		tags:
			'[{"Name":"Mammal","Confidence":99.278076171875,"Instances":[],"Parents":[{"Name":"Animal"}]},{"Name":"Pet","Confidence":99.278076171875,"Instances":[],"Parents":[{"Name":"Animal"}]},{"Name":"Canine","Confidence":99.278076171875,"Instances":[],"Parents":[{"Name":"Mammal"},{"Name":"Animal"}]},{"Name":"Dog","Confidence":99.278076171875,"Instances":[{"BoundingBox":{"Width":0.5215181112289429,"Height":0.9092578887939453,"Left":0.3343014717102051,"Top":0.08954165130853653},"Confidence":99.278076171875}],"Parents":[{"Name":"Pet"},{"Name":"Canine"},{"Name":"Animal"},{"Name":"Mammal"}]},{"Name":"Hound","Confidence":96.73748779296875,"Instances":[],"Parents":[{"Name":"Dog"},{"Name":"Pet"},{"Name":"Canine"},{"Name":"Animal"},{"Name":"Mammal"}]},{"Name":"Beagle","Confidence":85.5204086303711,"Instances":[],"Parents":[{"Name":"Hound"},{"Name":"Dog"},{"Name":"Pet"},{"Name":"Canine"},{"Name":"Animal"},{"Name":"Mammal"}]}]',
	},
];

exports.allFoundPets = allFoundPets;
exports.allLostPets = allLostPets;
exports.intergrationFoundPost = intergrationFoundPost;
exports.intergrationLostPost = intergrationLostPost;

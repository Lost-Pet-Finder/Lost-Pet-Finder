...
async function sendContactRequest(req, res) {
    ...
    try {
        ...
        const fcmRows = await sqlPool.query('CALL get_user_device_token(?)', [targetid]); 
        const deviceToken = fcmRows[0][0].device_token;

        const contactDataRows = await sqlPool.query('CALL get_user_contact_info(?)', [userid]);
...
async function respondToContactRequest(req, res) {
    ...
    try {
        if (accepted == true) {
            await sqlPool.query('CALL accept_contact_request(?,?)', [userid, requesterid]);

            
            // send other user a notification saying you accepted
            const fcmRows = await sqlPool.query('CALL get_user_device_token(?)', [requesterid]);
            ...
            await sqlPool.query('CALL delete_contact_request(?,?)', [userid, requesterid]);
        }
...
async function getMyPendingRequests(req, res) {
    ...
    try {
        const rows = await sqlPool.query('CALL get_my_pending_requests(?)', [userid]);
        const dataPackets = rows[0];

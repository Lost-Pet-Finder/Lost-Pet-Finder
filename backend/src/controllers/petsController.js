...
async function searchFoundPets(req, res) {
    ...
    try {
        const myReportRows = await sqlPool.query('CALL get_users_lost_reports(?)', [userid]);
        const userReport = myReportRows[0][0];
...
async function postFoundPets(req, res) {
    ...
    try {
        ...
        const response = await sqlPool.query('CALL create_found_pet_report(?, ?, POINT(?,?), ?, ?, ?)', [userid, filename, location_x, location_y, date, tagDataString, coloursArrayString]);
